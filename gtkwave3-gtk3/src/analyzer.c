 ...
 */
static void AddTrace(Trptr t, int select)
{
  GLOBALS->traces.dirty = 1;
  ...
  if (select) t->flags |= TR_HIGHLIGHT;
  ...
  if (!t->vector && GLOBALS->enum_nptrs_jrb)
  {
    ...
    if (enum_nptr)
    {
      ...
      if ((e_filter > 0) && (e_filter <= GLOBALS->num_xl_enum_filter))
      {
        t->e_filter = e_filter;
        if (!(GLOBALS->default_flags & TR_NUMMASK))
          t->flags = (t->flags & (~TR_NUMMASK)) | TR_ENUM | TR_BIN; /* need to downgrade to bin to make visible */
      }
    }
  }
  ...
  if (GLOBALS->default_flags & TR_TTRANSLATED)
  {
    ...
    if (t->t_filter)
    {
      ...
      {
        bvptr v;
        TraceFlagsType cache_hi = select ? TR_HIGHLIGHT : (t->flags & TR_HIGHLIGHT);
...
{
  return (AddNodeTraceReturn(nd, aliasname, NULL, select));
}
...
int AddNodeUnroll(nptr nd, char *aliasname, int select)
{
  ...
  else
  {
    ...
    {
      rc |= AddNodeTraceReturn(nd + i, aliasname, NULL, select);
    }
    return (rc);
  }
#endif
...
void UpdateTraceSelection(Trptr t)
{
  ...
  else if ((t->flags & (TR_BLANK | TR_ANALOG_BLANK_STRETCH)) && (GLOBALS->num_ttrans_filters)) /* seek to real xact trace if present... */
  {
    if (!(t->flags & TR_HIGHLIGHT))
    {
      Trptr tscan = t;
