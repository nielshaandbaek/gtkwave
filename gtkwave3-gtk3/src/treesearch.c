         ...
         (wrexm || (wrexm = wave_regex_match(t->name, WAVE_REGEX_TREE)))))
    {
      ...
      if (is_tname)
      {
        gtk_list_store_set(GLOBALS->sig_store_treesearch_gtk2_c_1, &iter,
                           ...
                           TREE_COLUMN, t,
                           TYPE_COLUMN,
                           (((GLOBALS->supplemental_datatypes_encountered) && (!GLOBALS->supplemental_vartypes_encountered)) ? (varxt ? varxt_pnt : vardatatype_strings[vardt]) : vartype_strings[vartype]),
  ...
  for (i = low; i <= high; i++)
  {
    ...
    if ((t) && (GLOBALS->autocoalesce))
    {
      if (get_s_selected(t))
      {
        ...
        if (len)
          add_vector_chain(s->vec_root, len, action == SST_ACTION_APPEND_AND_GROUP);
      }
    }
    else
    {
      AddNodeUnroll(s->n, NULL, action == SST_ACTION_APPEND_AND_GROUP);
    }
  }
}
...
sig_selection_foreach_finalize(gpointer data)
{
  ...
  if (action == SST_ACTION_APPEND_AND_GROUP)
  {
...
static void
sig_selection_foreach_preload_lx2(GtkTreeModel *model,
                                  GtkTreePath *path,
                                  GtkTreeIter *iter,
                                  gpointer data)
{
...
void action_callback(enum sst_cb_action action)
{
  ...
  gtk_tree_selection_selected_foreach(GLOBALS->sig_selection_treesearch_gtk2_c_1, &sig_selection_foreach_preload_lx2, (void *)action);
  ...
  gtk_tree_selection_selected_foreach(GLOBALS->sig_selection_treesearch_gtk2_c_1, &sig_selection_foreach, (void *)action);
...
do_tooltips:
  ...
  gtkwave_signal_connect(XXX_GTK_OBJECT(GLOBALS->dnd_sigview), "key_press_event", (GCallback)sig_view_cb, NULL);
...
void recurse_import(GtkWidget *widget, guint callback_action, int max_depth)
{
  if (GLOBALS->sst_sig_root_treesearch_gtk2_c_1)
  {
    ...
    if (GLOBALS->sst_sig_root_treesearch_gtk2_c_1->child)
      recurse_fetch_high_low(GLOBALS->sst_sig_root_treesearch_gtk2_c_1->child, max_depth);
    ...
    if ((GLOBALS->fetchlow >= 0) && (GLOBALS->fetchhigh >= 0))
    {
      ...
      {
        char recwarn[128];
        snprintf(recwarn, 128, "Really import %d facilit%s?", fz, (fz == 1) ? "y" : "ies");
